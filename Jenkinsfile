...
try {
     stage('Maven Build') {
      docker.image('maven:3.5-jdk-8-alpine').inside {
        ...
        sh "/bin/cp -f $WORKSPACE/Build-${env.BUILD_NUMBER}/vsvyadav_${env.BRANCH_NAME}${env.BUILD_NUMBER}.war $WORKSPACE/vsvyadav.war"
          ...
          stage('build image') {
        app = docker.build("parthu11/testing:docker${env.BUILD_NUMBER}")
       }
   ...
   stage('Deploy') {
        sh "/bin/cp -f $WORKSPACE/Build-${env.BUILD_NUMBER}/vsvyadav_${env.BRANCH_NAME}${env.BUILD_NUMBER}.war /opt/apache-tomcat-8.5.42/webapps/vsvyadav.war"
        sh " sh /opt/apache-tomcat-8.5.42/bin/startup.sh"
   }
